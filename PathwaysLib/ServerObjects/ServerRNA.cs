...
{	
	...
	/// </summary>
	#endregion
	public class ServerRNA : ServerGeneProduct
	{
...
//		/// <summary>
//		/// Creating constructor for the server rna wrapper
//		/// </summary>
//		/// <param name="name"></param>
//		/// <param name="molecular_entity_notes"></param>
//		/// <param name="gene_product_notes"></param>
//        /// <param name="rna_type"></param>
//        public ServerRNA (string name, string molecular_entity_notes, string gene_product_notes, string rna_type)
//			: base(name, "rnas", molecular_entity_notes, gene_product_notes)
//		{
//			// not yet in DB, so create empty row
//			__RNARow = new DBRow( __TableName );
//	             
//			this.ID = base.ID; // get ID generated by base class
//			this.RNAType = rna_type;
//		}
		...
		/// </param>
		public ServerRNA ( SoapRNA data )
			: base ( (SoapGeneProduct)data, "rnas")
		{
		...
		/// </param>
		public ServerRNA ( DBRow rnaRow ) 
			: base ( ServerGeneProduct.LoadRow(rnaRow.GetGuid("id") ) )
		{
					// (mfs)
					// setup object
			__RNARow = rnaRow;
		...
		public ServerRNA ( DBRow rnaRow, DBRow geneProductRow, DBRow molecularEntityRow ) 
		...
		public string RNAType
		{
			get
			{
				return RnaTypeManager.GetRnaTypeName(RNATypeId);
			}
			set
			{
				RNATypeId = RnaTypeManager.GetRnaTypeId(value); // (ac) fails if the string is not in the database
			}
		}
        ...
        /// Get/set RNA type ID.
        /// </summary>
		public int RNATypeId
		{
			get
			{
				return __RNARow.GetInt("rna_type_id");
			}
			set
			{
				__RNARow.SetInt("rna_type_id", value);
			}
		}
		#endregion
		...
		{
			SoapRNA retval = (derived == null) ? 
				retval = new SoapRNA() : retval = (SoapRNA)derived;

			// (BE) fill base class properties
			base.PrepareForSoap(retval);

			retval.ID   = this.ID;
			retval.RNAType = this.RNAType;
		...
		{
			// (sfa) Delete the rnas tuples
			DBWrapper.Instance.ExecuteNonQuery("DELETE FROM rnas WHERE id = @id"
		...
		protected override void SetSqlCommandParameters ( )
		{
			...
			__RNARow.ADOCommands["insert"] = DBWrapper.BuildCommand(
				"INSERT INTO " + __TableName + " (id, rna_type_id) VALUES (@i_id, @i_type_id);",
				"@i_id", SqlDbType.UniqueIdentifier, ID,
				"@i_type_id", SqlDbType.UniqueIdentifier, RNATypeId );			

			__RNARow.ADOCommands["select"] = DBWrapper.BuildCommand(
				"SELECT * FROM " + __TableName + " WHERE id = @s_id;",
				"@s_id", SqlDbType.UniqueIdentifier, ID );

			__RNARow.ADOCommands["update"] = DBWrapper.BuildCommand(
				"UPDATE " + __TableName + " SET rna_type_id = @u_type_id WHERE id = @u_id;",
				"@u_type_id", SqlDbType.UniqueIdentifier, RNATypeId,
				"@u_id", SqlDbType.UniqueIdentifier, ID );			

			__RNARow.ADOCommands["delete"] = DBWrapper.BuildCommand(
		...
		/// </returns>
		public static ServerRNA[] AllRNAs ( )
		{
			...
			{
				results.Add( new ServerRNA( new DBRow( d ) ) );
			}

			return ( ServerRNA[] ) results.ToArray( typeof( ServerRNA ) );
		}
		...
		/// <returns></returns>
		public static ServerRNA[] FindRNAs(string substring, SearchMethod searchMethod)
		{
		...
		/// </returns>
		public new static ServerRNA Load ( Guid id )
		{
			return new ServerRNA( LoadRow ( id ) );
		}
		...
		{
			return new ServerProtein(LoadRow(molecularEntityRow.GetGuid("id")), geneProductRow, molecularEntityRow);
		}
...
/*------------------------------- Change Log -----------------------------
	$Id: ServerRNA.cs,v 1.1 2008/05/16 21:15:53 mustafa Exp $
	$Log: ServerRNA.cs,v $
	...
	*** empty log message ***
	...
	Revision 1.15  2006/05/23 18:29:32  greg
	...
	Revision 1.12  2006/05/10 02:40:17  ali
	...
	Revision 1.11  2005/10/31 06:10:17  fatih
	*** empty log message ***
	...
	Revision 1.7  2005/07/18 19:18:12  brandon
	...
	Revision 1.6  2005/07/18 17:23:39  brandon
	Added queries:
	...
	Revision 1.5  2005/07/15 22:28:00  brendan
	Fix to automatically generate missing protein table entries from entries in molecular entities.
	
	Revision 1.4  2005/07/15 21:02:00  brandon
	added more queries
	
	Revision 1.3  2005/07/12 04:03:22  brandon
