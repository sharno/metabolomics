...
namespace PathwaysLib.WebControls
{
	...
	public class CollapsiblePanel : System.Web.UI.WebControls.Panel
	{
        ...
        private bool showNoItemImage = false;
		/// <summary>
        ...
        public CollapsiblePanel(bool showNoItemImage) 
        {
            this.CssClass = "itemdisplay";
            this.showNoItemImage = showNoItemImage;
        }
		...
		protected override void Render( HtmlTextWriter output )
		{
            ...
            else if (showNoItemImage)
            {
                output.WriteBeginTag("img");
                output.WriteAttribute("src", noitemimage);
                output.WriteAttribute("id", this.ID + "Image");
                output.WriteAttribute("class", "collapsebutton");                
                output.Write(HtmlTextWriter.SelfClosingTagEnd);
                output.Write(" ");
			...
			if( autoload || forcedautoload )
			{
				...
				output.WriteAttribute( "onload", string.Format( "{4}('{0}','{1}','{2}','{3}')",
					this.ID, controlName, PaginationHelper.ArgumentList( arguments ), collapseType,
					(forcedautoload ? "toggleRegion" : "addToQueue") ) );
