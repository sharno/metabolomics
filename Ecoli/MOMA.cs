...
namespace Ecoli
{
    class MOMA
    {
        public static bool Calculate(HyperGraph graph, IDictionary<string, double> preValues, INumExpr fobj)
        {
            ...
            {
                if(edge.Label.StartsWith("cycle")) continue;
                if (edge.IsPseudo)
                {
                ...
                var lb = Db.Context.ReactionBounds.Single(e => e.reactionId == edge.Id).lowerBound;
                var ub = Db.Context.ReactionBounds.Single(e => e.reactionId == edge.Id).upperBound;
