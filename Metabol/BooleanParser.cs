                ...
                while (_tokens.Current != null && _operators.Contains(_tokens.Current.Item2))
                {
                    ...
                    switch (operand.Item2)
                    {
                        case Token.And:
                            ...
                            Constraints.Add(cplex.IfThen(cplex.Eq(exp, 2), cplex.Eq(varr, 1), $"tand_{varname}"));
                            Constraints.Add(cplex.IfThen(cplex.Le(exp, 1), cplex.Eq(varr, 0), $"fand_{varname}"));

                            Constraints.Add(cplex.IfThen(cplex.Eq(varr, 1), cplex.Eq(exp, 2), $"tand2_{varname}"));
                            Constraints.Add(cplex.IfThen(cplex.Eq(varr, 0), cplex.Le(exp, 1), $"fand2_{varname}"));

                            left = varr;
                            break;
                        case Token.Or:

                            Constraints.Add(cplex.IfThen(cplex.Ge(exp, 1), cplex.Eq(varr, 1), $"tor_{varname}"));
                            Constraints.Add(cplex.IfThen(cplex.Eq(exp, 0), cplex.Eq(varr, 0), $"for_{varname}"));

                            Constraints.Add(cplex.IfThen(cplex.Eq(varr, 1), cplex.Ge(exp, 1), $"tor2_{varname}"));
                            Constraints.Add(cplex.IfThen(cplex.Eq(varr, 0), cplex.Eq(exp, 0), $"for2_{varname}"));
