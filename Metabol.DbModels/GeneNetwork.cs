...
namespace Metabol.DbModels
{
    public class GeneNetwork
    {
        ...
        static GeneNetwork()
        {
            ...
            //foreach (var gene in ecoli.genes)
            //    names.Add(gene.name.ToString().ToLower());

            //File.WriteAllLines("A:\\names.txt", names);
        ...
        public static ConcurrentDictionary<string, INumVar> AddRegulationConstraints(Cplex cplex, HyperGraph graph, Dictionary<Guid, INumVar> v)
        {
            ...
            foreach (var edge in graph.Edges.Values.Where(e => !e.IsPseudo && ridname.ContainsKey(e.Label)))
            {
                ...
                if (Math.Abs(edge.PreFlux) > 0.001)
                    cplex.Add(cplex.Eq(p.RootVar, 1));
                //else
                //    cplex.Add(cplex.Eq(p.RootVar, 0));

            }

            var net = LoadGraph(RootDir + "ecoli_network_tf_gene.txt");
            ...
            //cplex.Add(cplex.Eq(bvars["arca"], 1));
            //cplex.Add(cplex.Eq(bvars["h-ns"], 1));
            //cplex.Add(cplex.Eq(bvars["crp"], 1));
            //cplex.Add(cplex.Eq(bvars["fur"], 1));
            //cplex.Add(cplex.Eq(bvars["fnr"], 1));
